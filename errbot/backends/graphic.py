...
class CommandBox(QtGui.QPlainTextEdit):
    ...
    def updateCompletion(self, commands):
        ...
        self.completer = QCompleter(
            [(self.prefix + name).replace("_", " ", 1) for name in commands], self
        )
    ...
    def keyPressEvent(self, *args, **kwargs):
        ...
        if (
            ...
            and not (
                ...
                and self.textCursor().position() > self.autocompleteStart
            )
        ):
        ...
        if key == Qt.Key_Space and (ctrl or alt):
            ...
            self.completer.complete(
                rect
            )  # The popup is positioned in the next if block
        ...
        if key == Qt.Key_Up:
            if self.history_index > 0:
                ...
                self.setPlainText(
                    f'{self.prefix}{" ".join(self.history[self.history_index])}'
                )
                return
...
class ChatApplication(QtGui.QApplication):
    ...
    def new_message(self, text, receiving=True):
        ...
        self.buffer += (
            f'<div class="{"receiving" if receiving else "sending"}">{bot if receiving else user}'
            f"<br/>{text}</div>"
        )
    ...
    def scroll_output_to_bottom(self):
        self.output.page().mainFrame().scroll(
            0, self.output.page().mainFrame().scrollBarMaximum(QtCore.Qt.Vertical)
        )
...
class GraphicBackend(TextBackend):
    ...
    def build_message(self, text):
        ...
        return (
            msg  # rebuild a pure html snippet to include directly in the console html
        )
