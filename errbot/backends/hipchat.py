...
def hipchat_html():
    return Markdown(
        output_format="xhtml", extensions=[ExtraExtension(), HipchatExtension()]
    )
...
class HipChatRoom(Room):
    ...
    def create(self, privacy="public", guest_access=False):
        ...
        if self.exists:
            log.debug(
                "Tried to create the room %s, but it has already been created", self
            )
        else:
            self.hypchat.create_room(
                name=self.name, privacy=privacy, guest_access=guest_access
            )
    ...
    def notify(self, message, color=None, notify=False, message_format=None):
        ...
        self.room.notification(
            message=message, color=color, notify=notify, format=message_format
        )
...
class HipchatClient(XMPPConnection):
    def __init__(self, *args, **kwargs):
        ...
        if self.endpoint is None:
            ...
            self.hypchat = hypchat.HypChat(
                self.token, endpoint=self.endpoint, verify=verify
            )
...
class HipchatBackend(XMPPBackend):
    ...
    def callback_message(self, msg):
        ...
        people = list(
            filter(
                None.__ne__,
                [
                    self._find_user(mention[1:], "mention_name")
    ...
    def build_reply(self, msg, text=None, private=False, threaded=False):
        response = super().build_reply(
            msg=msg, text=text, private=private, threaded=threaded
        )
    ...
    def send_card(self, card):
        ...
        if not card.is_group:
            raise ValueError(
                "Private notifications/cards are impossible to send on 1 to 1 messages on hipchat."
            )
        log.debug("room id = %s", card.to)
        room = self.query_room(str(card.to)).room

        data = {
            "message": "-" if not card.body else self.md.convert(card.body),
            "notify": False,
            "message_format": "html",
        }
        ...
        if card.image and not card.summary and not card.fields and not card.link:
            hcard["style"] = "image"
            hcard["thumbnail"] = {
                "url": card.image if not card.thumbnail else card.thumbnail
            }
            ...
            if card.body:
                data[
                    ...
                    card.body
                )  # We don't have a card body field so retrofit it to the main body.
            ...
            if card.image or card.thumbnail:
                hcard["icon"] = {
                    "url": card.thumbnail if card.thumbnail else card.image
                }
            if card.body:
                hcard["description"] = card.body
            if card.summary:
            ...
            if card.fields:
                hcard["attributes"] = [
                    {
                        "label": key,
                        "value": {"label": value, "style": "lozenge-complete"},
                    }
    ...
    def _hipchat_upload(self, stream):
        ...
        try:
            ...
            img.add_header(
                "Content-Disposition", "attachment", name="file", filename=stream.name
            )
            raw_body.attach(img)
