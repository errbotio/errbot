...
class SlackPerson(Person):
    ...
    def username(self):
        ...
        if self._info:
            return (
                self._info['profile'].get('display_name_normalized') or
                self._info['profile'].get('real_name_normalized')
            )
        try:
            user = self._sc.users_info(user=self._userid)
            if user is None:
                raise SlackApiError
    ...
    def email(self):
        ...
        if self._info:
            return self._info.get('profile', {}).get('email', '')
...
class SlackRoomOccupant(RoomOccupant, SlackPerson):
    ...
    def __init__(self, sc, userid, channelid, bot):
        if isinstance(channelid, SlackRoom):
            super().__init__(sc, userid, channelid.id)
...
class SlackEventsBackend(ErrBot):
    ...
    def serve_forever(self):
        ...
        self.bot_identifier = SlackBot(self.sc, self.auth["user_id"], self.auth["user"])

        # Setup webhook to Errbot flask
        callable_view = WebView.as_view(self._dispatch_slack_message.__name__ + '_POST', self._dispatch_slack_message, None, True)
        flask_app.add_url_rule(self.slack_event_webhook, view_func=callable_view, methods=('POST', ), strict_slashes=False)
    ...
    def _dispatch_slack_message(self, request):
        ...
        """
        if not request.json:
            log.warn("Request received is not JSON.")
            return
    ...
    def _user_change_event_handler(self, event):
        ...
        #if user:
        #    idd = SlackPerson(self.sc, user['id'])
    ...
    def _message_event_handler(self, event):
        ...
        if channel.startswith('D'):
            if subtype == "bot_message":
                ...
                msg.to = SlackPerson(self.sc, user, event['channel'])
            else:
            ...
            if subtype == "bot_message":
                ...
                if user == self.bot_identifier.userid:
                    ...
                    msg.to = SlackRoom(channelid=event['channel'], bot=self)
                    msg.frm = SlackRoomOccupant(self.sc, user, msg.to, bot=self)
    ...
    def username_to_userid(self, name):
        ...
        try:
            user = self.sc.users_info(user=name)
            if user is None:
                raise SlackApiError
            return user['user']['id']
        except SlackApiError as e:
    ...
    def channels(self, exclude_archived=True, joined_only=False, types=room_types):
        ...
        while next_results:
            ...
            channels.extend([channel for channel in response['channels']
...
class SlackRoom(Room):
    ...
    def _channel_info(self):
        ...
        if self._info:
            return self._info
        try:
            info = self.sc.conversations_info(channel=self.id)
        except SlackApiError:
            return dict()
        if info['ok']:
            self._info = info['channel']
            return self._info
        return dict()
