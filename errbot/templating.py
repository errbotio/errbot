...
def add_plugin_templates_path(plugin_info: PluginInfo):
    ...
    if tmpl_path.exists():
        log.debug("Templates directory found for %s plugin [%s]", plugin_info.name, tmpl_path)
        ...
        return
    log.debug(
        "No templates directory found for %s plugin in [%s]", plugin_info.name, tmpl_path
    )
