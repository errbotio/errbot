...
if PY3:
    deps = ['setuptools', 'sleekxmpp', 'yapsy', 'bottle', 'requests']  # requests are for the unittests
    if py_version < (3, 2):
        raise RuntimeError(
            'On Python 3, Err requires Python 3.2 or later')
else:
    deps = ['configparser', 'setuptools', 'sleekxmpp', 'dnspython', 'yapsy', 'python-daemon', 'config', 'bottle', 'requests']
    if py_version < (2, 7):
        raise RuntimeError(
