...
if PY3:
    deps = ['webtest', 'setuptools', 'yapsy', 'bottle', 'rocket', 'requests', 'jinja2']  # requests are for the unittests
    if py_version < (3, 2):
        raise RuntimeError(
            'On Python 3, Err requires Python 3.2 or later')
else:
    deps = ['webtest', 'setuptools', 'yapsy', 'python-daemon', 'config', 'bottle', 'rocket', 'requests', 'jinja2']
    if py_version < (2, 7):
        raise RuntimeError(
